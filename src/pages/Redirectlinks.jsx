...
const DynamicLinkBuilder = () => {
  ...
  const deleteLink = async (id) => {
    ...
    try {
      const response = await fetch(`${API_URL}/api/surveylink/links/${id}`, {
        method: "DELETE"
      });
  ...
  const handleLinkClick = async (link) => {
    if (!link.isActive) {
      showNotification("This link is disabled and cannot be accessed!", "error");
      return;
    }
    ...
    try {
      await fetch(`${API_URL}/api/surveylink/links/${link._id}/click`, {
        method: "POST"
      });
      
      setLinks(links.map(l => 
        l._id === link._id ? { ...l, clicks: (l.clicks || 0) + 1 } : l
      ));
    } catch (error) {
