...
const StatsCard = ({ title, value, icon, trend }) => (
  <div className="main-page-stat-card">
    ...
    <p className="main-page-stat-value">{value.toLocaleString()}</p>
  </div>
);

// ⭐ UPDATED — Proportional BarChart Component
const BarChartComponent = ({ stats }) => {
  const data = [
    { label: 'Completed', value: stats.completedCount, color: '#10b981' },
    { label: 'Terminated', value: stats.terminatedCount, color: '#ef4444' },
    { label: 'Quota Full', value: stats.quotaFullCount, color: '#f59e0b' }
  ];
...
const MainPage = () => {
  ...
  const fetchStats = async () => {
    setLoading(true);
    try {
      const res = await fetch(`${API_URL}/api/dashboard/dashboard?filter=${filter}`);
      ...
      <div className="main-page-wrapper">
        <div className="main-page-container">
          ...
          <div className="main-page-stats-grid">
            <StatsCard title="Today Clicks" value={stats.today.todayClicks} icon="🖱️" />
            <StatsCard title="Today Completed" value={stats.today.todayCompleted} icon="✔️" />
            <StatsCard title="Today Terminated" value={stats.today.todayTerminated} icon="❌" />
            <StatsCard title="Today Quota Full" value={stats.today.todayQuotaFull} icon="🔒" />
          </div>

          {/* Overall Stats Section */}
          <h2 className="main-page-section-title">Overall Statistics</h2>
          <div className="main-page-stats-grid">
            <StatsCard title="Total Surveys" value={stats.totalSurveys} icon="📋" trend={12} />
            <StatsCard title="Total Clicks" value={stats.totalClicks} icon="👆" trend={8} />
            <StatsCard title="Total Submissions" value={stats.totalSubmissions} icon="✅" trend={15} />
          </div>
